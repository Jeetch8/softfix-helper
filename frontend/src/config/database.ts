...
export async function connectDB(retries = 3): Promise<boolean> {
  ...
  try {
    ...
    if (hostnameMatch) {
      ...
      try {
        ...
        console.warn(`⚠️  DNS resolution failed: ${dnsError?.message || String(dnsError)}`);
      }
    }
...
export async function disconnectDB(): Promise<void> {
  try {
    ...
    console.error('❌ Error disconnecting MongoDB:', error instanceof Error ? error.message : String(error));
  }
}
